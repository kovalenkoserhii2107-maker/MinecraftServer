...
| Установка аддона `.mcaddon` | `npm run addon:install -- <файл> [--yes]` |
...
│                               # zip.mjs — чтение ZIP без внешнего unzip
│                               # install-world.mjs — установка карты .mcworld
│                               # install-addon.mjs — установка стороннего аддона
...
`tools/install-addon.mjs` ставит чужие паки рядом с нашим и дописывает их в
списки мира, не затирая уже включённые. Перед установкой печатает версии
Script API, capabilities и прочее из манифеста — по этим полям и решается
совместимость.

Наш пак не переопределяет ванильные сущности и предметы, поэтому конфликтует
с чужими аддонами редко. Но защита участков покрывает только блоки: снаряды,
взрывы и урон игрокам она не ловит — оружейные аддоны обходят приваты.

Чужие паки в репозиторий не добавляются: у них своя лицензия, обычно
запрещающая распространение и изменение.
