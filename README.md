...
| --- | --- | --- |
| `device` | **Коммуникатор** — предмет управления, выдаётся каждому при входе | `/mc:device` |
| `economy` | **Криптогривна**: баланс, стартовые 100 000, табло справа | `/mc:balance` |
| `plots` | **Участки** 16×16 и 32×32: покупка, защита, налоги, карта | `/mc:claim`, `/mc:unclaim`, `/mc:plotinfo` |
| `blueprints` | **Чертежи**: голограмма габарита и постройка типовых зданий | `/mc:blueprint`, `/mc:bplist`, `/mc:bpsave`, `/mc:bpdelete` |
...
Основной способ управления сервером — **коммуникатор**: предмет, который каждый
игрок получает при входе. Взять в руку, нажать «Использовать» — откроется панель.
Через неё же будут управляться все механики, которые появятся дальше.
...
Потерялся (например, механика была выключена) — вернуть можно командой
`/mc:device`. Команда `/mc:panel` открывает ту же панель без предмета.
...
3. `/mc:bpsave <название> <цена>` — область сохраняется как структура мира и
   появляется в списке для всех игроков.

Сторона области ограничена 64 блоками (`maxSize`). Удалить — `/mc:bpdelete`.
