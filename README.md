...
```text
[ PS5 / Nintendo Switch ]
      │  Поиск LAN-игр (широковещательный UDP)
      ▼
[ Mac: Phantom proxy ] ── проброс UDP 19132 ──► [ Bedrock Dedicated Server ]
                                                 ├── Фаза 1: Docker на Mac (127.0.0.1)
                                                 └── Фаза 2: VPS DigitalOcean (Frankfurt)
```

Консоли не умеют подключаться к серверу по произвольному IP и порту, но ищут
игры в локальной сети. [Phantom](https://github.com/jhead/phantom) отвечает на
эти запросы от имени удалённого сервера и проксирует трафик, поэтому сервер
...
- Node.js 20+
- Mac и консоль в одной сети Wi-Fi
...
Phantom должен работать всё время игры: закроете терминал — консоли потеряют сервер.

### 7. Подключение с консоли
...
sudo -u mcdeploy -s
cd /opt/mc-server
cp .env.example .env && nano .env     # укажите OPS и SERVER_NAME
docker compose up -d
...
Затем `./tools/phantom.sh`. Консоли снова увидят сервер в списке LAN-игр —
на этот раз облачный.
...
**Сервер не виден на консоли**
- Phantom запущен и не выдал ошибок?
- Mac и консоль в одной сети Wi-Fi? Гостевая сеть и изоляция клиентов
  (AP isolation) на роутере ломают LAN-обнаружение.
- Порт проброшен: `docker compose ps` показывает `19132/udp`.
