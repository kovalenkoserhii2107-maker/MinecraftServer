...
`core/war.ts` — машина состояний (подготовка → бой → итоги), `mechanics/war.ts`
применяет последствия: режимы игры, уведомления, отсчёт, отмену урона.

Состояние хранится **только в памяти**, без dynamic properties. Война длится
минуты, а восстановление её из половины сохранённых данных после перезапуска
оставило бы игроков в чужом режиме игры.

Урон отменяется в `world.beforeEvents.entityHurt`. Для снаряда `damagingEntity`
— это стрелявший, поэтому оружие сторонних аддонов закрывается тем же правилом.

Взрывы фильтруются через `setImpactedBlocks`, а не отменяются целиком: эффект
остаётся видимым, но чужие блоки не ломаются.

## Цвета в интерфейсе

Кнопки форм рисуются на **светлом** фоне, тело формы — на тёмном. Поэтому в
подписях кнопок используйте палитру `Button` из `core/format.ts` (тёмные тона),
а в теле формы — `Color` (светлые). Светлый текст на кнопке нечитаем.
